...
pub fn mmul_f64(
    #[spirv(global_invocation_id)] global_id: UVec3,
    #[spirv(storage_buffer, descriptor_set = 0, binding = 0)] mat_a: &mut [f64],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 1)] mat_b: &mut [f64],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 2)] mat_c: &mut [f64],
    #[spirv(uniform, descriptor_set = 0, binding = 3)] matrix_dims: &UVec3,
) {
    mmul(global_id, mat_a, mat_b, mat_c, matrix_dims);
}
